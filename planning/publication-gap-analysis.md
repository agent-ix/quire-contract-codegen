...
| FND-905 | medium | Retained platform boundary: portable non-empty-directory replacement is rollback-atomic for reported failures, not process-crash atomic between two renames. | NFR-001, MP-001 |
...
all injectable failure points, modified files, unmarked and extra entries, empty directories, file
plus destination symlinks, and cleanup of a raced sibling symlink without following it. `cargo
