...
pub fn generate_enum_strategy(
    ...
    let body = match request.campaign {
        EnumStrategyCampaign::Broad => format!(
            "    proptest::sample::select(vec![{members}])\n        .prop_map(|current| {case_type} {{ current, related: None, expected: {domain_type}::Accepted }})\n        .boxed()"
        ),
        EnumStrategyCampaign::StatePinned { variant } => format!(
            "    proptest::strategy::Just({case_type} {{ current: {}::{variant}, related: None, expected: {domain_type}::Accepted }}).boxed()",
            request.enum_type
        ),
        EnumStrategyCampaign::NoEvent { variant } => format!(
            "    proptest::strategy::Just({case_type} {{ current: {}::{variant}, related: Some({}::{variant}), expected: {domain_type}::Accepted }}).boxed()",
...
\n\
/// Admission expectation for an enum case drawn from the declared membership.\n\
///\n\
/// Enum strategy populations contain admissible members only; unlike integer boundary\n\
/// populations, they do not synthesize out-of-membership customer enum values.\n\
#[derive(Clone, Copy, Debug, Eq, PartialEq)]\n\
pub enum {domain_type} {{\n\
    /// The enum case belongs to the declared membership.\n\
    Accepted,\n\
}}\n\
\n\
impl {domain_type} {{\n\
    /// Returns false because emitted enum populations contain admissible members only.\n\
    #[must_use]\n\
    pub const fn expects_rejection(self) -> bool {{\n\
        false\n\
    }}\n\
\n\
    ...
    pub fn verify(\n\
        ...
        if matches {{\n\
            ...
            Err(proptest::test_runner::TestCaseError::fail(format!(\n\
                \"generated enum expected-domain mismatch: expected={{self:?}} actual={{verdict:?}}\"\n\
            )))\n\
        }}\n\
    }}\n\
}}\n\
...
pub struct {case_type} {{\n\
    ...
    pub related: Option<{}>,\n\
    /// Admission expectation for this declared enum member.\n\
    pub expected: {domain_type},\n\
}}\n\
\n\
        ...
        StrategyConstraint::ResidualExclusion {
                    ...
                    .any(|value| *value < minimum || *value > maximum)
            {
                Err(diagnostic(
                    StrategyErrorCode::InvalidMembership,
                    "constraint.excluded",
                    "residual exclusions must contain unique members of the base range",
                ))
            } else {
                Ok(())
            }
...
fn strategy_body(
    ...
    match campaign {
        ...
        StrategyCampaign::Broad => Ok(broad_body(constraint, case_type, domain_type)),
...
fn boundary_body(
    ...
    let cases: BTreeSet<(i64, Option<i64>)> = match constraint {
        StrategyConstraint::InclusiveRange { minimum, maximum } => {
            let mut cases = BTreeSet::new();
            cases.insert((minimum, None));
            cases.insert((maximum, None));
            if let Some(value) = minimum.checked_sub(1) {
                cases.insert((value, None));
            }
        ...
        StrategyConstraint::Membership { values } => {
            let mut cases = BTreeSet::new();
            cases.extend(values.iter().copied().map(|value| (value, None)));
            for value in values {
                if let Some(adjacent) = value.checked_sub(1) {
                    cases.insert((adjacent, None));
                }
        ...
        StrategyConstraint::CorrelatedOffset {
            ...
            for primary in [primary_minimum, primary_maximum] {
                for offset in [offset_minimum, offset_maximum] {
                    if let Some(related) = primary.checked_add(offset) {
                        cases.insert((primary, Some(related)));
                    }
                }
            }
            ...
            {
                cases.insert((primary_minimum, Some(related)));
            }
            if let Some(related) = primary_maximum
                .checked_add(offset_maximum)
                .and_then(|value| value.checked_add(1))
            {
                cases.insert((primary_maximum, Some(related)));
            }
            cases
        }
    };

    let has_accepted = cases
        .iter()
        .any(|(primary, related)| constraint_accepts(constraint, *primary, *related));
    let has_rejected = cases
        .iter()
        .any(|(primary, related)| !constraint_accepts(constraint, *primary, *related));
    if !has_accepted || !has_rejected {
        return Err(diagnostic(
            StrategyErrorCode::UnsupportedCampaignConstraint,
            "campaign.boundary",
            "the representable boundary population must contain accepted and rejected cases",
        ));
    }
    ...
    let entries = cases
        ...
        .join(", ");
    Ok(format!(
        "    proptest::sample::select(vec![{entries}]).boxed()"
    ))
}
...
fn expected_domain(
    ...
    let accepted = constraint_accepts(constraint, primary, related);
    format!(
        "{domain_type}::{}",
...
fn enforce_source_limit(source: &str) -> Result<(), StrategyDiagnostic> {
