...
| FR-016 | FR-016-AC-1 through FR-016-AC-8 | TC-026 | 🚧 Partial: the witness join (`witness_schema`, `decode_falsification`) implements AC-8 and the harness-identity half of AC-5, and part of AC-1: in the default suite, a matching decode and the arity and width refusals (`src/kani_witness_join.rs` unit tests, `tests/it/kani_argument_order.rs`); against a real Kani falsification, decode and the arity and width refusals in the ignored Kani lane (`tests/it/kani_witness_join.rs`). The Boolean-byte and comment refusals are exercised only by quire-contract-ir's own `Witness::decode` tests, and the join reports them as `KaniOutcome` refusal codes rather than FR-016's malformed-witness replay result. Binding to the harness pins (AC-5), the decode size limit (AC-6), domain validation (AC-2) and native replay (AC-3, AC-4, AC-7) are planned |
