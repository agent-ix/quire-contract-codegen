...
FR-001-AC-8 adds (agent-ix/quire-contract-codegen#4). SL lowers a model scalar such as
ConfigVersion's `VersionNumber` (0..=1000) into `IntegerType { domain, minimum, maximum, overflow }`
on each `ValueDeclaration`.

## Terms

- **Read**: one `ValueReference` operand, identified by its declaration `SymbolName` and its
  `StateObservation`.
...
- The clause's `ClauseKind`, `DeclarationEnvironment` values, and `TypedExpression`.
...
- If the clause reads one declaration through `Current` and also through `Pre` or `Post`, then the
  generator shall refuse with `UnsupportedRelation`, because this slice does not define that aliasing.
...
- Every refusal diagnostic shall carry the full `ClauseRef` and the exact IR source span of the
  offending node, using the optional span field that agent-ix/quire-contract-codegen#4 adds to
  interface-001 `diagnostics.fields`.
- The generator shall report every new refusal as a variant of the existing `StrategyErrorCode` in
  `StrategyDiagnostic`.
- The generator shall treat the two reads of one comparison as sharing one domain, because the IR
  types both operands of a `Compare` with the identical `IntegerType`.

## Constraints

| ID | Constraint | Type | Validation |
|----|------------|------|------------|
| FR-008-CON-1 | The generator SHALL read domain bounds only through the public `quire_contract_ir` API at the pinned revision, with no private decoder or local copy of the IR wire shape. | Interface | Inspection |
...
|----|----------|--------------|
| FR-008-AC-1 | Given ConfigVersion `VersionUnchanged`, the census lists `versionNumber` as a `State` declaration read at `Post` (primary) and `Pre` (partner) with inclusive domain 0..=1000 taken from its `IntegerType`. | Test (TC-017) |
...
| FR-008-AC-4 | An `Assertion` or `Case` clause is refused with `UnsupportedClauseKind`, and an absent `ClauseRef` is refused with `UnknownClause` and `invalid-input`; every refusal carries the full `ClauseRef` and span. | Test (TC-017) |
...
- **Upstream**: [StR-001](../../stakeholder/StR-001-traceable-generation.md);
  ...
  (agent-ix/quire-contract-codegen#4, specified at `task/4-numeric-state-oracles` revision `393c6a1`);
