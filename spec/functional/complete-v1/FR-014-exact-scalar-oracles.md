...
`convert`) and not the operation identity (for example which division law,
decimal rounding, or IEEE rounding). The operation identity is therefore a
caller-declared typed descriptor that the generator cross-checks against the
lowered node. A transport that carries operation identity is an upstream gap,
recorded as a blocked item for this slice.
...
- A typed claim map with one entry per requested item: node id, Contract IR
  semantic id, package id, semantic type, source map, claims, bounds, operation
  identity, and either the generated symbol or the typed refusal.
...
- If a node is unlowered, invalid, unbounded, over its work limit, or
  disagrees with its descriptor, then the generator shall return that item's
  typed refusal and emit no code for it, without changing any other item's
  output.
...
- If a node id appears more than once in the request, then the generator shall
  refuse every copy.
- The generator shall order output by node digest so that equal requests in
  any order produce identical bytes.

## Constraints

| ID | Constraint | Type | Validation |
|----|------------|------|------------|
| FR-014-CON-1 | Generated oracles SHALL NOT contain a charge amount; every charge comes from runtime metering. | Integrity | Test |
| FR-014-CON-2 | Generated oracle functions SHALL NOT panic; invalid generated constants surface as a typed stop. | Integrity | Test |
...
| FR-014-AC-3 | A descriptor whose arity or result scalar type disagrees with the lowered node, a node that is not a lowered scalar expression, or a node id requested more than once, is refused with a typed reason. | Test (TC-024) |
| FR-014-AC-4 | Generated bytes are identical across repeated runs and across permutations of the request order, and match the committed golden output. | Test (TC-024) |
...
| FR-014-AC-8 | The generated crate declares `publish = false`, pins the runtime revision with the `exact` feature, contains no charge literal, and compiles. | Test (TC-024) |
