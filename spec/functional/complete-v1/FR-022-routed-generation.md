...
| FR-022-AC-13 | The Kani arm routes an integer `add` over two bounded parameters, `Int[0, 9]` and `Int[10, 20]`, with a result bounded `[0, 29]`, to a supported `quire.op.integer.add` harness whose arguments are `[0, 9]` and `[10, 20]`, one range per operand, and whose result assertion uses the result bound `[0, 29]`. | Test (TC-033) |
