...
- When generating for the Kani kind, the generator shall derive an FR-014
  ...
  item, a claim whose result is the FR-014 refusal `NoDerivableClaim` carrying
  the `ClaimDerivationRefusal`. It shall order the claims by node id.
...
| FR-022-AC-11 | An item with no derivable descriptor keeps its typed refusal and no harness, and its siblings' records and harnesses are unaffected. | Test (TC-033) |
| FR-022-AC-12 | `RoutedGeneration.claim_map` is `Some` after a Kani group and equals the FR-014 claim map over the derived items, with one `NoDerivableClaim` claim per underivable node, ordered by node id. | Test (TC-033) |
...
5. **Follow-up tickets.** Linear IR-295 tracks FR-019's Kani settlement
   ...
   tracks a node whose closure carries two bounds of one form on its result
   type, which derivation reports as `AmbiguousBound`. *Blocks:* nothing here.
