...
TC-006 has primitive and bound-observation tests, including a nested native LLVM fixture. That
historical test profile uses cargo-llvm-cov 0.9.0 with Rust 1.94.1 and LLVM full JSON export format
3.0.1 with per-file segments; it does not qualify the required Rust 1.98.1 profile. The analyzer
consumes export bytes and never invokes LLVM, Cargo, Quire, or Quoin. The producer version and LLVM
export-format version are separate facts. A positive
...
revision. The implemented bound analyzer retains unqualified provenance. Native producer/run
binding, shared retained-result integration and current-profile qualification remain incomplete;
existing analysis cannot discharge those obligations.
